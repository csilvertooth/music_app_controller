...
const MUSIC_CONTROLLER_JS_VERSION = '2025-09-08-0.7.2';
...
// Lightweight shared store to cache state across views and keep SSE alive
// even when individual cards/panels are not mounted. This improves
// responsiveness when navigating and ensures background updates.
(() => {
  try {
    const g = (typeof window !== 'undefined') ? window : globalThis;
    if (g.__appleMusicStore) return; // singleton
    const listeners = new Set();
    const notify = (event, data) => { for (const fn of Array.from(listeners)) { try { fn({ event, data }); } catch(_){} } };
    const setLS = (k, v) => { try { localStorage.setItem(k, JSON.stringify(v)); } catch(_){} };
    const getLS = (k, dflt) => { try { const v = localStorage.getItem(k); return v ? JSON.parse(v) : dflt; } catch(_) { return dflt; } };
    ...
    const state = {
      ...
      sseHealthy: false,
      playerState: getLS('apple_music_store_state', ''), // 'playing' | 'paused' | 'idle'
      ...
      try {
        if (event === 'snapshot') {
          ...
          state.now = {
            ...
            artwork_token: (data && data.artwork_token) || (now && now.artwork_token) || state.now.artwork_token || '',
          };
          if (typeof now.state === 'string' && now.state) state.playerState = String(now.state).toLowerCase();
          state.nowTs = Date.now();
          if (typeof data.master === 'number') state.master = data.master;
          const air = Array.isArray(data.airplay) ? data.airplay : null;
          if (air) {
            state.devices = air.map(d => d && d.name).filter(Boolean);
            state.current = new Set(air.filter(d => d && d.active).map(d => d.name));
            const nv = {}; air.forEach(d => { if (d && typeof d.volume === 'number') nv[d.name] = Math.max(0, Math.min(1, d.volume/100)); });
            state.vols = nv;
          }
          persist();
        ...
        if (event === 'now' || event === 'status' || event === 'now_playing' || event === 'now-playing') {
          state.now = {
            title: (data && data.title) || state.now.title || '',
            artist: (data && data.artist) || state.now.artist || '',
            album: (data && data.album) || state.now.album || '',
            artwork_token: (data && (data.artwork_token || data.token)) || state.now.artwork_token || '',
          };
          if (data && typeof data.state === 'string') state.playerState = String(data.state).toLowerCase();
          state.nowTs = Date.now();
          if (Object.prototype.hasOwnProperty.call(data||{}, 'shuffle')) state.shuffle = !!data.shuffle;
          persist();
          notify('now', data);
          return;
        }
        if (event === 'shuffle') {
          state.shuffle = !!(data && data.enabled);
          persist();
          notify('shuffle', data);
          return;
        }
        ...
        if (event === 'airplay_full') {
          ...
          state.devices = list.map(d => d && d.name).filter(Boolean);
          state.current = new Set(list.filter(d => d && d.active).map(d => d.name));
          const nv = {}; list.forEach(d => { if (d && typeof d.volume === 'number') nv[d.name] = Math.max(0, Math.min(1, d.volume/100)); });
          state.vols = { ...state.vols, ...nv };
          persist();
        ...
        if (event === 'artwork_saved') {
          try { notify('artwork_saved', data); } catch(_) {}
          return;
        }
      } catch(_) { /* ignore */ }
    };
    ...
    const openSSE = () => {
      ...
      const getSigned = async () => {
        try {
          const hass = (g && g.hassConnection && g.hassConnection.hass) ? g.hassConnection.hass : (g && g.hass);
          if (hass && hass.callWS) {
            const resp = await hass.callWS({ type: 'auth/sign_path', path: '/api/apple_music/events', expires: 60 });
            if (resp && resp.path) return resp.path;
          }
        } catch(_){}
        return '/api/apple_music/events';
      };
      (async () => {
        ...
        src.onopen = () => {
          ...
          try { prefetch(true); } catch(_){}
        };
        src.onmessage = (ev) => {
          try { const msg = JSON.parse(ev.data); applyEvent(msg.event, msg.data); state.sseHealthy = true; } catch(_){}
        };
        src.onerror = () => {
    ...
    const prefetch = async (force=false) => {
      try {
        let hass = (g && g.hassConnection && g.hassConnection.hass) ? g.hassConnection.hass : (g && g.hass); // best-effort
        try { if (!hass) { const el = document.querySelector('home-assistant'); if (el && el.hass) hass = el.hass; } } catch(_){}
        const call = async (m,p,b) => {
          try {
            if (hass && hass.callApi) return await hass.callApi(m||'GET', p, b);
            if (!hass) return null; // avoid unauth 401s before hass is ready
        ...
        // Normalize devices/current/volumes first
        let names = [];
        if (devs) {
        ...
        if (vols && typeof vols === 'object') {
          const n = {}; for (const [k,v] of Object.entries(vols)) { let val = Number(v); if (!isFinite(val)) continue; if (val > 1.5) val = val/100; if (val < 0) val = 0; if (val > 1) val = 1; n[k] = val; }
        ...
        if (st) {
          ...
          if (Object.prototype.hasOwnProperty.call(now, 'shuffle')) state.shuffle = !!now.shuffle;
          try {
          ...
          try { state.nowTs = Date.now(); } catch(_){ }
        }
        persist();
        // Synthesize a combined AirPlay payload first to avoid flicker
        try {
        ...
        // Also notify individual streams for compatibility if needed
        try { notify('devices', state.devices); } catch(_){ }
        try { notify('current_devices', Array.from(state.current)); } catch(_){ }
        try { notify('device_volumes', state.vols); } catch(_){ }
        try { notify('now', state.now); } catch(_){ }
      } catch(_) { /* ignore */ }
    };

    // Start background fetch + SSE immediately since this script is loaded globally
    prefetch(true);
    openSSE();
    if (!pollFast) pollFast = setInterval(() => prefetch(false), 5000);

    g.__appleMusicStore = {
      // subscribe to receive {event, data} updates; returns unsubscribe function
      subscribe(fn) { if (typeof fn === 'function') { listeners.add(fn); return () => { try { listeners.delete(fn); } catch(_){} }; } return () => {}; },
      snapshot() { return { devices: state.devices.slice(), current: new Set(state.current), vols: { ...state.vols }, now: { ...state.now }, now_ts: state.nowTs || 0, shuffle: !!state.shuffle, master: state.master, sseHealthy: !!state.sseHealthy, state: state.playerState || '' }; },
      prefetch,
    };
  } catch(_) { /* ignore store init errors */ }
})();
...
class MusicControllerPanel extends HTMLElement {
  constructor() {
    ...
    this._storeUnsub = null;   // unsubscribe function for shared store
    ...
    try {
      ...
      try {
        ...
        if (store) {
          ...
          // Now Playing
          const now = snap.now || {};
          const nowTs = Number(snap.now_ts || 0);
          ...
          try { if (snap.state) this._updatePlayPauseVisual?.(snap.state); } catch(_){ }
          // Devices snapshot
          try {
  ...
  _wireStaticHandlers() {
    ...
    document.addEventListener('click', (e)=>{
      ...
      toggleMenu(false); toggleAbout(false); toggleServices(false);
    }, { passive:true });
    ...
    this.querySelector('#menuRefreshArtwork')?.addEventListener('click', async () => { toggleMenu(false); try { await this._hass.callService('apple_music','refresh_current_artwork',{}); alert('Artwork refreshed.'); } catch(_) { alert('Failed to refresh artwork.'); } });
    this.querySelector('#menuPurgeAlbumServer')?.addEventListener('click', async () => { toggleMenu(false); if (!confirm('Purge server album cache?')) return; try { await this._purgeAlbumCache(); alert('Server album cache purged.'); } catch(_) { alert('Failed to purge on server.'); } });
    this.querySelector('#menuPurgeAlbumHA')?.addEventListener('click', async () => { toggleMenu(false); if (!confirm('Purge Home Assistant album cache?')) return; try { await this._hass.callService('apple_music','purge_ha_album_cache',{}); alert('Home Assistant album cache purged.'); } catch(_) { alert('Failed to purge HA cache.'); } });
  ...
  _startPolling() {
    ...
    if (!this._sseHealthy) {
      ...
      // With a healthy SSE/store, rely on push and a slow watchdog without forcing an immediate poll
  ...
  async _fetchDevices(force=false) {
    // When SSE/store is healthy, skip redundant fetches unless forced
    if (this._sseHealthy && !force) return;
    try {
      const list = await this._fetchJSON('apple_music/devices');
      let names = [];
      if (Array.isArray(list)) {
        names = list.map(d => (typeof d === 'string') ? d : (d && d.name)).filter(Boolean);
      } else if (list && Array.isArray(list.devices)) {
        names = list.devices.map(d => (typeof d === 'string') ? d : (d && d.name)).filter(Boolean);
      }
      if (names.length || force) this._devices = names;
    } catch (e) { /* ignore */ }
  }

  async _fetchCurrentDevices() {
    // When SSE/store is healthy, rely on push updates to avoid flicker
    if (this._sseHealthy) return;
    try {
      const cur = await this._fetchJSON('apple_music/current_devices');
      let arr = Array.isArray(cur) ? cur : (cur?.devices || []);
      const names = arr.map(d => (typeof d === 'string') ? d : (d && (d.name || d.device))).filter(Boolean);
      if (!this._pendingApply) this._currentDevices = new Set(names);
    } catch (e) { /* ignore */ }
  }
  ...
  _renderDevices() {
    ...
    // Only show active devices by default. Show disabled only when toggled.
    const showAll = this._showDisabled;
    ...
    // If not showing disabled and there are no active devices, render a friendly hint
    if (!showAll && order.length === 0) {
      const el = document.createElement('div');
      el.className = 'card device';
      el.innerHTML = `<div style="padding: 12px; opacity:.8;">No active AirPlay devices. Click \u201cShow Disabled\u201d to enable devices.</div>`;
      frag.appendChild(el);
    }
...
class MusicNowPlayingCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    // Hook shared store once to seed/cache updates even if card was recreated
    try {
      const store = (typeof window !== 'undefined') ? window.__appleMusicStore : null;
      if (store && !this._usingStore) {
        this._usingStore = true;
        // Seed from snapshot immediately
        try {
          const snap = store.snapshot();
          const now = snap.now || {};
          const nowTs = Number(snap.now_ts || 0);
          const trackEl = this._root.querySelector('#track'); if (trackEl && now.title) trackEl.textContent = now.title;
          const artistEl = this._root.querySelector('#artist'); if (artistEl && now.artist) artistEl.textContent = now.artist;
          const albumEl = this._root.querySelector('#album'); if (albumEl && now.album) albumEl.textContent = now.album;
          try { this._lastNPKey = `${now.title||''}|${now.artist||''}|${now.album||''}`; } catch(_){ }
          const tok = now.artwork_token || '';
          if (tok && (!nowTs || (Date.now() - nowTs) < 10000)) { try { this._setArtworkToken(tok); } catch(_){} }
          const btn = this._root.querySelector('#shuffle'); if (btn) btn.classList.toggle('active', !!snap.shuffle);
          // Apply cached play/pause state immediately
          try {
            const st = (snap.state || '').toLowerCase();
            const pbtn = this._root.querySelector('#playpause');
            if (pbtn) { pbtn.classList.remove('pp-playing','pp-paused','pp-idle'); if (st==='playing') pbtn.classList.add('pp-playing'); else if (st==='paused') pbtn.classList.add('pp-paused'); else pbtn.classList.add('pp-idle'); }
          } catch(_){ }
        } catch(_){}
        // Subscribe to future updates
        this._storeUnsub = store.subscribe(({ event, data }) => {
          try {
            if (event === 'now' || event === 'status' || event === 'now_playing' || event === 'now-playing') {
              const title = (data && data.title) || '';
              const artist = (data && data.artist) || '';
              const album = (data && data.album) || '';
              if (title) { const el = this._root.querySelector('#track'); if (el) el.textContent = title; }
              if (artist) { const el = this._root.querySelector('#artist'); if (el) el.textContent = artist; }
              if (album) { const el = this._root.querySelector('#album'); if (el) el.textContent = album; }
              try { this._lastNPKey = `${title}|${artist}|${album}`; } catch(_){ }
              const tok = (data && (data.artwork_token || data.token)) || '';
              if (tok) { try { this._setArtworkToken(tok); } catch(_){} }
              return;
            }
            if (event === 'artwork_saved') {
            ...
            if (event === 'shuffle') {
              const btn = this._root.querySelector('#shuffle'); if (btn) btn.classList.toggle('active', !!(data && data.enabled));
              return;
            }
            if (event === 'master_volume') {
              const slider = this._root.querySelector('#masterVol'); if (slider && typeof data === 'number' && Date.now() >= this._holdUntil) slider.value = String(Math.max(0, Math.min(1, data/100)));
              return;
            }
          } catch(_){}
        });
...
MusicNowPlayingCard.prototype._artCacheKey = function(tok){
  try { const meta = this._lastNPKey || ''; if (meta) return `https://am-art.local/art/by-meta/${encodeURIComponent(meta)}`; } catch(_){}
  return `https://am-art.local/art/${encodeURIComponent(tok||'current')}`;
};
...
MusicNowPlayingCard.prototype._fetchAndCacheArt = async function(tok){
  ...
  let resp = null;
  try { resp = await fetch(url, { cache: 'no-store', credentials: 'same-origin' }); } catch(_) { resp = null; }
  if (resp && resp.ok) {
...
class MusicAirplayOutputsCard extends HTMLElement {
  ...
  connectedCallback() {
    // Prefer global store if present to avoid duplicate connections and enable caching across views
    try {
      const store = (typeof window !== 'undefined') ? window.__appleMusicStore : null;
      if (store && !this._usingStore) {
        this._usingStore = true;
        ...
        try {
          const snap = store.snapshot();
          this._devices = Array.isArray(snap.devices) ? snap.devices : [];
          this._current = (snap.current instanceof Set) ? new Set(Array.from(snap.current)) : new Set([]);
          this._vols = (snap.vols && typeof snap.vols === 'object') ? { ...snap.vols } : {};
          this._render();
        } catch(_){ }
        ...
        this._storeUnsub = store.subscribe(({ event, data }) => {
          try {
            if (event === 'airplay_full') {
              const list = Array.isArray(data) ? data : [];
              this._devices = list.map(d => d && d.name).filter(Boolean);
              this._current = new Set(list.filter(d => d && d.active).map(d => d.name));
              const nv = {}; list.forEach(d => { if (d && typeof d.volume === 'number') nv[d.name] = Math.max(0, Math.min(1, d.volume/100)); });
              this._vols = { ...this._vols, ...nv };
              this._render();
              return;
            }
            if (event === 'devices' || event === 'airplay_devices') {
              const list = Array.isArray(data) ? data : (data && Array.isArray(data.devices) ? data.devices : []);
              const names = list.map(d => (typeof d === 'string') ? d : (d && d.name)).filter(Boolean);
              if (names.length) { this._devices = names; this._render(); }
              return;
            }
            if (event === 'current_devices' || event === 'selected_devices') {
              let arr = [];
              if (Array.isArray(data)) arr = data; else if (data && Array.isArray(data.devices)) arr = data.devices;
              const names = arr.map(d => (typeof d === 'string') ? d : (d && (d.name || d.device))).filter(Boolean);
              this._current = new Set(names);
              this._render();
              return;
            }
            if (event === 'device_volumes') {
              if (data && typeof data === 'object' && !Array.isArray(data)) {
                this._vols = { ...this._vols, ...data };
                this._render();
              }
              return;
            }
        ...
        // Lightweight initial fetch in background (store may fill it faster)
        try { store.prefetch && store.prefetch(false); } catch(_){}
        return; // skip local polling/SSE
      }
    } catch(_) {}

    // Fallback: start local polling + SSE when store is not available
  ...
  async _fetchDevices(force=false) {
    if (this._usingStore && !force) return;
    try {
      const list = await this._apiGet('apple_music/devices');
      let names = [];
      if (Array.isArray(list)) names = list.map(d => (typeof d === 'string') ? d : (d && d.name)).filter(Boolean);
      else if (list && Array.isArray(list?.devices)) names = list.devices.map(d => (typeof d === 'string') ? d : (d && d.name)).filter(Boolean);
      this._devices = names;
    } catch (e) { /* ignore */ }
  }
  async _fetchCurrent() {
    if (this._usingStore) return;
    try {
      const cur = await this._apiGet('apple_music/current_devices');
      const arr = Array.isArray(cur) ? cur : (cur?.devices || []);
      const names = arr.map(d => (typeof d === 'string') ? d : (d && (d.name || d.device))).filter(Boolean);
      if (!this._pending) this._current = new Set(names);
    } catch (e) { /* ignore */ }
  }
  async _fetchVolumes() {
    try {
  ...
  _connectSSE() {
    // If using the shared store, skip opening a local EventSource
    if (this._usingStore) return;
    try {
      // Always allow connecting; background throttling handled by browser
      const open = (path) => {
...
MusicControllerPanel.prototype._connectSSE = function() {
  try {
    // Prefer shared global store if available to avoid duplicate EventSource
    const store = (typeof window !== 'undefined') ? window.__appleMusicStore : null;
    if (store) {
      // Seed latest snapshot to UI quickly
      try {
        const snap = store.snapshot();
        this._applySnapshot({ now: snap.now, now_ts: snap.now_ts, master: snap.master, airplay: (snap.devices||[]).map(n=>({ name:n, active: snap.current && snap.current.has && snap.current.has(n), volume: Math.round(((snap.vols||{})[n]||0)*100) })) });
        // Apply cached shuffle/play state
        try { this._status = { ...(this._status||{}), shuffle: !!snap.shuffle }; this._updateShuffleButtonVisual?.(); } catch(_){ }
        try { if (snap.state) this._updatePlayPauseVisual?.(snap.state); } catch(_){ }
      } catch(_){}
      // Subscribe to store updates and reuse SSE handler
      if (this._storeUnsub) { try { this._storeUnsub(); } catch(_){} this._storeUnsub = null; }
      this._storeUnsub = store.subscribe(({ event, data }) => {
        try { this._handleSSE({ event, data }); this._sseHealthy = true; } catch(_){}
      });
      // Keep only slow watchdog polling when using store
      this._sseHealthy = true;
      if (this._pollHandle) { clearInterval(this._pollHandle); this._pollHandle = null; }
...
MusicControllerPanel.prototype._applySnapshot = function(data) {
  ...
  // Prefer entity text if it disagrees with snapshot to reduce stale flashes
  const ent = this._mpEntity?.();
  const attrs = ent?.attributes || {};
  const entTitle = attrs.media_title || '';
  const entArtist = attrs.media_artist || '';
  const entAlbum = attrs.media_album_name || '';
  const snapTitle = String(now.title || '');
  const snapArtist = String(now.artist || '');
  const snapAlbum = String(now.album || '');
  const entKey = `${entTitle}|${entArtist}|${entAlbum}`;
  const snapKey = `${snapTitle}|${snapArtist}|${snapAlbum}`;
  const useEntityMeta = ent && entKey && snapKey && entKey !== snapKey;
  const nextTitle = useEntityMeta ? (entTitle || t?.textContent || '—') : (snapTitle || t?.textContent || '—');
  const nextArtist = useEntityMeta ? (entArtist || ar?.textContent || '—') : (snapArtist || ar?.textContent || '—');
  const nextAlbum = useEntityMeta ? (entAlbum || al?.textContent || '—') : (snapAlbum || al?.textContent || '—');
  if (t) t.textContent = nextTitle;
  if (ar) ar.textContent = nextArtist;
  if (al) al.textContent = nextAlbum;
  try { this._lastNPKey = `${nextTitle}|${nextArtist}|${nextAlbum}`; } catch(_){ }
  // Only set artwork from snapshot if it looks recent and matches entity metadata
  const atok = data.artwork_token || now.artwork_token || now.token;
  const ts = typeof data.now_ts === 'number' ? data.now_ts : 0;
  const isRecent = !ts || (Date.now() - ts) < 10000; // 10s freshness window
  if (atok && isRecent && (!useEntityMeta)) this._setArtwork?.(atok);
  ...
  // AirPlay devices snapshot
  try {
    const air = Array.isArray(data.airplay) ? data.airplay : [];
    if (air.length) {
      this._devices = air.map(d => d && d.name).filter(Boolean);
      this._currentDevices = new Set(air.filter(d => d && d.active).map(d => d.name));
      const vols = {}; air.forEach(d => { if (typeof d.volume === 'number') vols[d.name] = Math.max(0, Math.min(1, d.volume/100)); });
      this._deviceVolumes = vols;
      this._renderDevices?.();
    }
  } catch(_){ }
...
MusicControllerPanel.prototype._artCacheKey = function(tok){
  try { const meta = this._lastNPKey || ''; if (meta) return `https://am-art.local/art/by-meta/${encodeURIComponent(meta)}`; } catch(_){}
  return `https://am-art.local/art/${encodeURIComponent(tok||'current')}`;
};
