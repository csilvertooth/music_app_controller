...
MusicControllerPanel.prototype._setArtwork = async function(tok) {
  try {
    ...
    const tmp = new Image();
    tmp.onload = () => { if (this._currentArtTok === tok) { const prev = img.dataset.blobUrl; img.src = url; img.dataset.blobUrl = url; if (prev && prev !== url) { try { URL.revokeObjectURL(prev); } catch(_){} } } else { try { URL.revokeObjectURL(url); } catch(_){} } };
