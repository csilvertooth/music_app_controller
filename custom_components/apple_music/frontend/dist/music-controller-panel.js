export class MusicControllerPanel extends HTMLElement {
    ...
    _handleSSEEvent(data) {
        ...
        switch (event) {
            ...
            case 'snapshot':
                ...
                // Handle shuffle/repeat state updates from snapshot (similar to individual events)
                if (payload.shuffle && Date.now() >= this._shuffleHold) {
                    const enabled = !!((_a = payload.shuffle) === null || _a === void 0 ? void 0 : _a.enabled);
                    this._status = Object.assign(Object.assign({}, (this._status || {})), { shuffle: enabled });
                    (_b = this._updateShuffleButtonVisual) === null || _b === void 0 ? void 0 : _b.call(this);
                }
                if (payload.repeat && Date.now() >= this._repeatHold) {
                    const mode = ((_c = payload.repeat) === null || _c === void 0 ? void 0 : _c.mode) || 'off';
                    this._status = Object.assign(Object.assign({}, (this._status || {})), { repeat: mode });
                    (_d = this._updateRepeatButtonVisual) === null || _d === void 0 ? void 0 : _d.call(this);
                }
                break;
            case 'airplay_full':
            ...
            case 'shuffle':
                try {
                    // Respect user hold to prevent rapid flashing
                    const now = Date.now();
                    if (now < this._shuffleHold)
                        break;
                    const enabled = !!(payload === null || payload === void 0 ? void 0 : payload.enabled);
                    this._status = Object.assign(Object.assign({}, (this._status || {})), { shuffle: enabled });
                    (_e = this._updateShuffleButtonVisual) === null || _e === void 0 ? void 0 : _e.call(this);
                }
                catch (_) { }
                break;
            case 'repeat':
                try {
                    // Respect user hold to prevent rapid flashing
                    const now = Date.now();
                    if (now < this._repeatHold)
                        break;
                    const mode = (payload === null || payload === void 0 ? void 0 : payload.mode) || 'off';
                    this._status = Object.assign(Object.assign({}, (this._status || {})), { repeat: mode });
                    (_f = this._updateRepeatButtonVisual) === null || _f === void 0 ? void 0 : _f.call(this);
                }
                catch (_) { }
                break;
        }
    ...
    _updateRepeatButtonVisual() {
        try {
            const btn = this.querySelector('#repeat');
            if (!btn)
                return;
            const mode = String((this._status && this._status.repeat) || 'off');
            btn.classList.remove('repeat-off', 'repeat-one', 'repeat-all');
            btn.classList.add(`repeat-${mode}`);
        }
        catch (_) { }
    }
    ...
    _updateNowPlayingAux() {
        // Sync shuffle and repeat button states from backend when possible
        try {
            if (!this._hass || !this._hass.callApi)
                return;
            const hass = this._hass; // Type-safe reference
            // Protect against overriding recently changed shuffle state
            const shuffleNow = Date.now();
            if (shuffleNow > this._shuffleHold) {
                hass.callApi('GET', 'apple_music/shuffle').then((res) => {
                    // Double-check hold hasn't been reset during async call
                    if (Date.now() < this._shuffleHold)
                        return;
            ...
            // Protect against overriding recently changed repeat state
            const repeatNow = Date.now();
            if (repeatNow > this._repeatHold) {
                hass.callApi('GET', 'apple_music/repeat').then((res) => {
                    // Double-check hold hasn't been reset during async call
                    if (Date.now() < this._repeatHold)
                        return;
                    const mode = (res === null || res === void 0 ? void 0 : res.mode) || 'off';
                    this._status = Object.assign(Object.assign({}, (this._status || {})), { repeat: mode });
                    this._updateRepeatButtonVisual();
                }).catch(() => { });
            }
        }
        catch (_) { }
        ...
        try {
            ...
            try {
                ...
                if (store) {
                    ...
                    // Shuffle + play/pause (repeat is not persisted in shared store)
                    try {
            ...
            try {
                ...
                if (missing && this._hass && this._hass.callApi) {
                    // Protect against overriding user-changed repeat state
                    const now = Date.now();
                    const shouldProtect = now < this._repeatHold;
                    if (shouldProtect && this._status) {
                        // Skip fetching state if user recently changed repeat
                    }
                    else {
    ...
    _renderSkeleton() {
        const style = `
          ...
          /* Two-row controls layout for better mobile compatibility */
          .controls-row-primary { display:flex; gap:8px; margin-top:12px; justify-content:center; }
          .controls-row-secondary { display:flex; gap:8px; margin-top:8px; justify-content:center; }
          .controls-row-secondary button { padding:8px 12px; font-size:0.85rem; }
          ...
          /* Repeat visual states */
          #repeat { background: var(--secondary-background-color, color-mix(in srgb, var(--primary-text-color) 12%, transparent)); color: var(--primary-text-color, CanvasText); }
          #repeat.repeat-all { background: var(--primary-color); color: #fff; }
          #repeat.repeat-one { background: #FF9800; color: #000; }
          #repeat.repeat-off { background: var(--secondary-background-color, color-mix(in srgb, var(--primary-text-color) 12%, transparent)); color: var(--primary-text-color, CanvasText); }
              ...
              <div class="media">
                ...
                <div class="meta">
                  ...
                  <div class="controls-row-secondary">
                    ...
                    <button id="repeat" class="secondary"><ha-icon icon="mdi:repeat"></ha-icon><span class="btnlabel">Repeat</span></button>
                  </div>
                  <div class="volrow">
                    <span style="opacity:.8">Vol</span>
    ...
    _wireStaticHandlers() {
        ...
        (_c = this.querySelector('#repeat')) === null || _c === void 0 ? void 0 : _c.addEventListener('click', () => this._toggleRepeat());
        (_d = this.querySelector('#shuffle')) === null || _d === void 0 ? void 0 : _d.addEventListener('click', () => this._toggleShuffle());
        (_e = this.querySelector('#next')) === null || _e === void 0 ? void 0 : _e.addEventListener('click', () => this._mpService('media_next_track'));
        (_f = this.querySelector('#mvDown')) === null || _f === void 0 ? void 0 : _f.addEventListener('click', () => this._bumpVol(-0.05));
        (_g = this.querySelector('#mvUp')) === null || _g === void 0 ? void 0 : _g.addEventListener('click', () => this._bumpVol(0.05));
        ...
        (_h = this.querySelector('#openBrowse')) === null || _h === void 0 ? void 0 : _h.addEventListener('click', () => this._showBrowse());
        // AirPlay Outputs
        (_j = this.querySelector('#refreshOutputs')) === null || _j === void 0 ? void 0 : _j.addEventListener('click', () => this._poll(true));
        (_k = this.querySelector('#toggleDisabled')) === null || _k === void 0 ? void 0 : _k.addEventListener('click', () => {
            var _a;
        ...
        // Browse controls
        (_l = this.querySelector('#browsePlaylists')) === null || _l === void 0 ? void 0 : _l.addEventListener('click', () => this._setBrowseSection('playlists'));
        (_m = this.querySelector('#browseAlbums')) === null || _m === void 0 ? void 0 : _m.addEventListener('click', () => this._setBrowseSection('albums'));
        (_o = this.querySelector('#browseArtists')) === null || _o === void 0 ? void 0 : _o.addEventListener('click', () => this._setBrowseSection('artists'));
        (_p = this.querySelector('#browseSearch')) === null || _p === void 0 ? void 0 : _p.addEventListener('input', (e) => this._setBrowseSearch(e.target.value));
        (_q = this.querySelector('#browsePrev')) === null || _q === void 0 ? void 0 : _q.addEventListener('click', () => this._browsePrev());
        (_r = this.querySelector('#browseNext')) === null || _r === void 0 ? void 0 : _r.addEventListener('click', () => this._browseNext());
        // Letter buttons
        this.querySelectorAll('.letterbtn').forEach(btn => {
            btn.addEventListener('click', (e) => this._setBrowseAlpha(e.target.dataset.letter || ''));
        });
        // Settings menu
        (_s = this.querySelector('#settingsBtn')) === null || _s === void 0 ? void 0 : _s.addEventListener('click', () => this._toggleSettingsMenu());
        (_t = this.querySelector('#menuCustomize')) === null || _t === void 0 ? void 0 : _t.addEventListener('click', () => this._showLayoutBar());
        (_u = this.querySelector('#menuReset')) === null || _u === void 0 ? void 0 : _u.addEventListener('click', () => this._resetLayout());
        // Sidebar toggle (HA drawer)
        (_v = this.querySelector('#menu')) === null || _v === void 0 ? void 0 : _v.addEventListener('click', () => {
            try {
        ...
        // Settings actions
        (_w = this.querySelector('#menuRestart')) === null || _w === void 0 ? void 0 : _w.addEventListener('click', () => {
            var _a;
            try {
        ...
        (_x = this.querySelector('#menuPurgeAlbumServer')) === null || _x === void 0 ? void 0 : _x.addEventListener('click', () => {
            var _a;
            try {
        ...
        (_y = this.querySelector('#menuPurgeAlbumHA')) === null || _y === void 0 ? void 0 : _y.addEventListener('click', () => {
            var _a;
            try {
        ...
        (_z = this.querySelector('#menuRefreshArtwork')) === null || _z === void 0 ? void 0 : _z.addEventListener('click', () => {
            var _a;
            try {
    ...
    _poll(force = false) {
        ...
        calls.push(this._hass.callApi('GET', 'apple_music/status').then((st) => {
            ...
            if (!st)
                return;
            // Protect shuffle/repeat from overwriting if user recently changed them
            const now = Date.now();
            const protectShuffle = now < this._shuffleHold;
            const protectRepeat = now < this._repeatHold;
            const newStatus = Object.assign({}, st);
            if (protectShuffle && this._status)
                newStatus.shuffle = this._status.shuffle;
            if (protectRepeat && this._status)
                newStatus.repeat = this._status.repeat;
            this._status = newStatus;
            (_a = this._updateShuffleButtonVisual) === null || _a === void 0 ? void 0 : _a.call(this);
            (_b = this._updateRepeatButtonVisual) === null || _b === void 0 ? void 0 : _b.call(this);
            (_c = this._updateFromHass) === null || _c === void 0 ? void 0 : _c.call(this);
            try {
        ...
        // Redundant individual GET calls removed - full status includes these with protection
    ...
    _toggleShuffle() {
        ...
        if (!this._hass)
            return;
        this._shuffleHold = Date.now() + 2000; // Set hold immediately
        ...
        this._hass.callApi('POST', 'apple_music/shuffle', { enabled: newShuffle })
            .then((_response) => {
            // Keep the hold for protection
            this._shuffleHold = Date.now() + 2000;
        })
            .catch((error) => {
            console.warn('Failed to toggle shuffle:', error);
            this._shuffleHold = 0; // Clear hold
    ...
    _toggleRepeat() {
        ...
        if (!this._hass)
            return;
        this._repeatHold = Date.now() + 2000; // Set hold immediately
        const currentMode = String(((_a = this._status) === null || _a === void 0 ? void 0 : _a.repeat) || 'off').toLowerCase();
        let newMode = 'off';
        if (currentMode === 'off')
            newMode = 'all';
            ...
            newMode = 'off';
        // Optimistically update UI
        this._status = Object.assign(Object.assign({}, (this._status || {})), { repeat: newMode });
        (_b = this._updateRepeatButtonVisual) === null || _b === void 0 ? void 0 : _b.call(this);
        this._hass.callApi('POST', 'apple_music/repeat', { mode: newMode })
            .then((_response) => {
            // Keep the hold for protection
            this._repeatHold = Date.now() + 2000;
        })
            .catch((error) => {
            var _a;
            console.warn('Failed to toggle repeat:', error);
            this._repeatHold = 0; // Clear hold
            // Refetch current state
            (_a = this._hass) === null || _a === void 0 ? void 0 : _a.callApi('GET', 'apple_music/repeat').then((res) => {
                var _a;
                const mode = (res === null || res === void 0 ? void 0 : res.mode) || 'off';
                this._status = Object.assign(Object.assign({}, (this._status || {})), { repeat: mode });
                (_a = this._updateRepeatButtonVisual) === null || _a === void 0 ? void 0 : _a.call(this);
            }).catch(() => { });
        });
    }
    _bumpVol(delta) {
