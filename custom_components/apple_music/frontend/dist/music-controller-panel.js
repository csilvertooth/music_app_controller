...
class MusicControllerPanel extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (!this._wakeHandlersBound) {
      this._onVis = () => { if (document.visibilityState === 'visible') { try { this._connectSSE(); } catch(_){} this._poll(true); } };
      this._onPageShow = () => { try { this._connectSSE(); } catch(_){} this._poll(true); };
      this._onFocus = () => { this._poll(true); };
      this._onOnline = () => { try { this._connectSSE(); } catch(_){} this._poll(true); };
