...
class MusicControllerPanel extends HTMLElement {
  ...
  _renderSkeleton() {
    const style = `
      ...
      @media (min-width: 1024px) { .sections { grid-template-columns: 1fr 1fr; } }
      ...
      .sections.edit ha-card.editable { outline: 2px dashed rgba(255,255,255,0.15); cursor: grab; }
      .sections.edit ha-card.editable:active { cursor: grabbing; }
      .resize-handle { position:absolute; right:6px; bottom:6px; width:16px; height:16px; border-right: 3px solid rgba(255,255,255,0.35); border-bottom:3px solid rgba(255,255,255,0.35); border-left: 3px solid transparent; border-top: 3px solid transparent; border-radius:2px; cursor: nwse-resize; opacity:.8; }
      .drag-placeholder { border:2px dashed var(--primary-color); border-radius:12px; min-height: 80px; background: transparent; }
      @media (min-width: 900px) { .art { width:480px; height:480px; } }
      ...
      @media (min-width: 800px) { .media { flex-wrap: nowrap; } }
      .appbar { position:sticky; top:0; z-index:1; display:flex; align-items:center; justify-content:space-between; gap:8px; padding:8px 12px; margin-bottom:12px; background: var(--card-background-color, #1f1f1f); border-radius:12px; box-shadow: var(--ha-card-box-shadow, none); }
      .appbar .left { display:flex; align-items:center; gap:8px; }
      .appbar button.icon { line-height:1; padding:6px 10px; border-radius:8px; background: var(--secondary-background-color, #444); color: var(--primary-text-color, #fff); }
      .appbar .apptitle { font-weight:600; }
      .appbar a.link { text-decoration:none; color: var(--primary-color); }
      .card { border-radius:12px; background: var(--card-background-color, #1f1f1f); box-shadow: var(--ha-card-box-shadow, none); padding:16px; min-width:260px; }
      .full { width:100%; }
      .title { font-size:1.1rem; font-weight:600; margin-bottom:8px; }
      .art { width:320px; height:320px; border-radius:8px; background:#333; object-fit:cover; }
      .meta { min-width:0; text-align:center; margin-inline:auto; max-width:100%; }
      ...
      .controls { display:flex; gap:12px; margin-top:12px; white-space: nowrap; justify-content:center; }
      ...
      button.active { box-shadow: 0 0 0 2px var(--primary-color); }
      ...
      #shuffle { background: var(--secondary-background-color, #444); color: var(--primary-text-color, #fff); }
      ...
      button.secondary { background: var(--secondary-background-color, #444); color: var(--primary-text-color, #fff); }
      ...
      .segbtn { cursor:pointer; border:none; border-radius:12px; padding:6px 10px; background: var(--secondary-background-color, #444); color: var(--primary-text-color, #fff); }
      ...
      .search input[type="search"] { flex: 1 1 180px; max-width: 320px; border-radius:8px; border:1px solid var(--divider-color, #555); background: transparent; color: var(--primary-text-color, #fff); padding:6px 10px; }
      .hint { opacity:.7; padding:4px 16px 0; }
      .list { padding:8px 16px 16px; display:grid; gap:8px; max-width:100%; overflow-x:hidden; }
      .item { display:flex; align-items:center; justify-content:space-between; gap:12px; padding:10px 12px; border-radius:10px; background: var(--secondary-background-color, #2a2a2a); flex-wrap: nowrap; cursor: pointer; min-height:56px; height:56px; box-sizing:border-box; overflow:hidden; }
      ...
      .item[data-act] { border:1px solid var(--divider-color, #555); background: var(--secondary-background-color, #2a2a2a); transition: background .15s ease, transform .02s ease-in; }
      .item[data-act]:hover { background: rgba(255,255,255,0.06); }
      .item[data-act]:active { transform: translateY(1px); }
      .item[data-act] .label::after { content: '›'; opacity:.6; margin-left:8px; }
      .item { cursor: pointer; }
      /* Tap/click feedback */
      @keyframes pressPulse { 0% { transform: scale(1); box-shadow: 0 0 0 0 rgba(255,255,255,0.15);} 50% { transform: scale(0.985); box-shadow: 0 0 0 2px rgba(255,255,255,0.10);} 100% { transform: scale(1); box-shadow: 0 0 0 0 rgba(255,255,255,0);} }
      ...
      .pager .letters .letterbtn { flex:0 0 auto; cursor:pointer; border:none; border-radius:999px; padding:4px 8px; background: var(--secondary-background-color, #444); color: var(--primary-text-color, #fff); font-size:12px; }
      ...
      .range-compact { width: clamp(160px, 40vw, 360px); }
      .device .dev-volume { width: 100%; max-width: 360px; }
    ...
    this.innerHTML = `
      ...
      <div class="wrap">
        <div class="appbar">
          ...
          <a class="link" href="/lovelace">Overview</a>
          <button id="customize" class="secondary" title="Customize layout">Customize</button>
        </div>
        <div id="layoutBar" class="layoutbar" style="display:none;">
          ...
          <div class="grp">
            ...
            <button data-key="now" data-act="size" class="secondary">Size</button>
            <button data-key="now" data-act="col" class="secondary">Col</button>
          </div>
          <div class="grp">
            <span>Outputs</span>
            <button data-key="outputs" data-act="up" class="secondary">↑</button>
            <button data-key="outputs" data-act="down" class="secondary">↓</button>
            <button data-key="outputs" data-act="size" class="secondary">Size</button>
            <button data-key="outputs" data-act="col" class="secondary">Col</button>
          </div>
          <div class="grp">
            <span>Browse</span>
            <button data-key="browse" data-act="up" class="secondary">↑</button>
            <button data-key="browse" data-act="down" class="secondary">↓</button>
            <button data-key="browse" data-act="size" class="secondary">Size</button>
            <button data-key="browse" data-act="col" class="secondary">Col</button>
          </div>
        </div>
  ...
  _wireStaticHandlers() {
    ...
    bar?.addEventListener('click', (ev) => {
      ...
      else if (act === 'size') this._cycleSize(key);
    });
    ...
    // Pager controls
    this.querySelector('#pageFirst')?.addEventListener('click', () => this._pageSet(0));
    this.querySelector('#pagePrev')?.addEventListener('click', () => this._pageSet(this._browsePage - 1));
    this.querySelector('#pageNext')?.addEventListener('click', () => this._pageSet(this._browsePage + 1));
    this.querySelector('#pageLast')?.addEventListener('click', () => this._pageSet(Number.POSITIVE_INFINITY));
  ...
  // ====== HA helpers ======
  _mpEntity() { return this._hass?.states?.['media_player.apple_music_player']; }
  ...
  _startPolling() {
    // Prefer SSE push; fallback to periodic polling if SSE fails
    this._connectSSE();
    // One eager pull to populate UI asap
    this._poll(true);
  }
  ...
  _renderDevices() {
    ...
    for (const name of order) {
      ...
      el.innerHTML = `
        ...
        <div>
          <div class="dev-volrow" style="display:flex; gap:8px; align-items:center;">
  ...
  async _browseLoadViaHA(section, arg) {
    ...
    const token = this._browseReqToken;
    const entity_id = 'media_player.apple_music_player';
  ...
  _pageSet(p) {
    if (!this._isTop(this._browseSection)) return;
    const items = this._getFilteredTopItems();
    const pages = Math.max(1, Math.ceil(items.length / this._pageSize));
    let next = p;
  ...
  async _playContainer(type, name, shuffle=false, idArg='') {
    const entity = 'media_player.apple_music_player';
...
class MusicNowPlayingCard extends HTMLElement {
    ...
    if (!this._root) {
      ...
      this._root.innerHTML = `
        <style>
          ...
          .art { width: clamp(160px, 50vw, 320px); aspect-ratio: 1 / 1; height: auto; border-radius:8px; background:#333; object-fit:cover; }
          ...
          .modal .sheet { position:relative; width: min(92vw, 860px); max-width: 860px; max-height: 80vh; margin: 0 16px; }
          .modal .close { position:absolute; top:8px; right:12px; z-index:1; }
          @keyframes pressPulseNP { 0% { transform: scale(1); box-shadow: 0 0 0 0 rgba(255,255,255,0.15);} 50% { transform: scale(0.985); box-shadow: 0 0 0 2px rgba(255,255,255,0.10);} 100% { transform: scale(1); box-shadow: 0 0 0 0 rgba(255,255,255,0);} }
  ...
  _entityId() { return this._config.entity || 'media_player.apple_music_player'; }
  _svc(service, data={}) {
    if (!this._hass) return;
  ...
  async _update() {
    ...
    if (artUrl) artEl.src = artUrl; else artEl.removeAttribute('src');
  ...
  setConfig(config) {
    ...
    if (!this._root) {
      ...
      this._root.innerHTML = `
        <style>
          .head { display:flex; align-items:center; justify-content:space-between; margin-bottom:8px; padding:0 16px; }
          .grid { display:grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap:16px; padding:0 16px 16px; }
          .card { border-radius:12px; background: var(--card-background-color, #1f1f1f); box-shadow: var(--ha-card-box-shadow, none); padding:16px; }
          .device { display:flex; flex-direction:column; gap:8px; }
          .switch { display:flex; align-items:center; gap:8px; }
...
class MusicBrowseCard extends HTMLElement {
    ...
    if (!this._root) {
      ...
      this._root.innerHTML = `
        <style>
          ...
          .item { display:flex; align-items:center; justify-content:space-between; gap:12px; padding:10px 12px; border-radius:10px; background: var(--secondary-background-color, #2a2a2a); cursor:pointer; min-height:56px; height:56px; box-sizing:border-box; overflow:hidden; }
          .item[data-act] { border:1px solid var(--divider-color, #555); transition: background .15s ease, transform .02s ease-in; }
          .item[data-act]:hover { background: rgba(255,255,255,0.06); }
          .item[data-act]:active { transform: translateY(1px); }
          .label { position:relative; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; flex: 1 1 0; min-width:0; }
          .label-inner { display:inline-block; will-change: transform; transform: translateX(0); }
      ...
      this._root.querySelector('#pageFirst')?.addEventListener('click', () => this._pageSet(0));
      this._root.querySelector('#pagePrev')?.addEventListener('click', () => this._pageSet(this._browsePage - 1));
      this._root.querySelector('#pageNext')?.addEventListener('click', () => this._pageSet(this._browsePage + 1));
      this._root.querySelector('#pageLast')?.addEventListener('click', () => this._pageSet(Number.POSITIVE_INFINITY));
  ...
  _pageSet(p){ if (!this._isTop(this._browseSection)) return; const items=this._getFilteredTopItems(); const pages=Math.max(1, Math.ceil(items.length/this._pageSize)); let next=p; if(!isFinite(next)||next>pages-1) next=pages-1; if(next<0) next=0; this._browsePage=next; this._renderBrowse(); }
...
MusicControllerPanel.prototype._connectSSE = function() {
  try {
    ...
    const es = new EventSource('/api/apple_music/events');
    this._es = es;
    es.onmessage = (ev) => {
      try {
...
MusicControllerPanel.prototype._handleSSE = function(msg) {
  ...
  if (event === 'now') {
    if (data) {
      ...
      const img = this.querySelector('#art'); if (img && data.artwork_token) img.src = `/api/apple_music/artwork?tok=${data.artwork_token}`;
