...
class MusicControllerPanel extends HTMLElement {
  ...
  _renderSkeleton() {
    const style = `
      ...
      /* Keep art and meta stacked until fairly wide to avoid cramps in mobile landscape */
      @media (min-width: 1024px) { .media { flex-wrap: nowrap; } }
      ...
      .art { width: clamp(140px, min(45vw, 60vh), 360px); aspect-ratio: 1 / 1; height: auto; border-radius:8px; background:#333; object-fit:cover; }
      ...
      #shuffle { background: var(--secondary-background-color, #2196F3); color: var(--primary-text-color, #fff); }
      #shuffle.active { background: #2196F3; color: #fff; }
      /* Play/Pause visual states */
      #playpause { background: var(--secondary-background-color, #444); color: var(--primary-text-color, #fff); }
      #playpause.pp-playing { background: #2196F3; color: #fff; }
      #playpause.pp-paused { background: #FB8C00; color: #1b1b1b; }
      #playpause.pp-idle { background: var(--secondary-background-color, #444); color: var(--primary-text-color, #fff); }
      ...
      .flash { background: #2196F3 !important; color:#fff !important; }
      ...
      <div class="wrap">
        <div class="appbar">
          ...
          <div class="right-actions" style="position:relative; display:flex; gap:8px; align-items:center;">
            ...
            <div id="settingsMenu" class="menu" style="display:none; position:absolute; right:0; top:36px; background: var(--card-background-color, #1f1f1f); border:1px solid var(--divider-color, #444); border-radius:10px; box-shadow: var(--ha-card-box-shadow, 0 4px 16px rgba(0,0,0,0.4)); min-width:200px; z-index:5;">
              ...
              <div style="height:1px; background: var(--divider-color, #444); margin:6px 0;"></div>
        ...
        <div id="layoutBar" class="layoutbar" style="display:none;">
          ...
          <div class="grp">
            <label style="display:flex; align-items:center; gap:6px;">
              ...
              <span>Link Now & Outputs width</span>
            </label>
          </div>
          <div class="grp">
            <button id="resetLayout" class="secondary" title="Reset layout">Reset</button>
          </div>
        </div>
  ...
  _wireStaticHandlers() {
    ...
    const toggleMenu = (show) => { if (!menu) return; menu.style.display = show ? '' : 'none'; };
    settingsBtn?.addEventListener('click', (e) => { e.stopPropagation(); const vis = menu && menu.style.display !== 'none'; toggleMenu(!vis); });
    document.addEventListener('click', (e)=>{ if (!menu || !settingsBtn) return; if (menu.contains(e.target) || settingsBtn.contains(e.target)) return; toggleMenu(false); }, { passive:true });
  ...
  _startPolling() {
    // Start SSE for push; keep periodic polling as a safety net
...
MusicControllerPanel.prototype._setArtwork = function(tok) {
  try {
    const path = `/api/apple_music/artwork?tok=${encodeURIComponent(tok)}`;
    this._signedPath(path).then((signed) => {
      const img = this.querySelector('#art');
      if (img) img.src = signed || path;
    });
  } catch (_) {}
};
