...
PLAYER_ENTITY_ID = "media_player.apple_music_player"  # adjust if your entity id differs
...
async def async_setup_entry(
    ...
    st = hass.states.get(PLAYER_ENTITY_ID)
    if st:
    ...
    async_track_state_change_event(hass, [PLAYER_ENTITY_ID], _player_state_changed)
...
class AppleMusicAirPlayVolume(NumberEntity):
    ...
    def suggested_object_id(self) -> str:
        return f"{_pretty_name(self._device_name)} volume"
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        await self.hass.services.async_call(
            DOMAIN,
            "set_device_volume",
            {"entity_id": PLAYER_ENTITY_ID, "device": self._device_name, "level": level},
            blocking=True,
        )
