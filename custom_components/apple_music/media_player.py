...
class AppleMusicAirPlayPlayer(MediaPlayerEntity):
    ...
    @property
    def suggested_object_id(self) -> str:
        # Clean AirPlay device entity ID: device_name + suffix
        # Remove any duplicate "airplay" in device name to avoid names like:
        # great_room_airplay_great_room_airplay_music_controller
        device_slug = slugify(self._device_name)
        if "airplay" in device_slug:
            return f"{device_slug}_music_controller"
        else:
            return f"{device_slug}_airplay_music_controller"
    ...
    async def async_update(self) -> None:
        ...
        try:
            ...
            # Sync individual device volume from the domain volume entities
            volume_bucket = self._hass.data.get(DOMAIN, {}).get("volume_entities") or {}
            volume_ent = volume_bucket.get(self._device_name)
            if volume_ent and hasattr(volume_ent, 'native_value'):
                vol = getattr(volume_ent, 'native_value', 50)
                if isinstance(vol, (int, float)):
