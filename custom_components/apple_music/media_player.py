...
class AppleMusicPlayer(MediaPlayerEntity):
    ...
    async def async_update(self) -> None:
        ...
        if not isinstance(repeat_state, Exception):
            try:
                now = time.monotonic()
                # Don't overwrite if user recently changed repeat (within last 3 seconds for better responsiveness)
                if (now - self._repeat_last_user_action) > 3.0:
        ...
        if not isinstance(shuffle_state, Exception):
            try:
                now = time.monotonic()
                # Don't overwrite if user recently changed shuffle (within last 2 seconds)
                if (now - self._shuffle_last_user_action) > 2.0:
...
class AppleMusicAirPlayPlayer(MediaPlayerEntity):
    ...
    async def async_repeat(self, repeat: RepeatMode) -> None:
        ...
        main_player = self._hass.data.get(DOMAIN, {}).get("player_ref")
        if main_player and hasattr(main_player, 'async_repeat'):
            await main_player.async_repeat(repeat)
        self.async_schedule_update_ha_state(True)
    ...
    async def async_shuffle(self, shuffle: bool) -> None:
        ...
        main_player = self._hass.data.get(DOMAIN, {}).get("player_ref")
        if main_player and hasattr(main_player, 'async_shuffle'):
            await main_player.async_shuffle(shuffle)
        self.async_schedule_update_ha_state(True)
