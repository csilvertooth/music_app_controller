...
class AppleMusicAirPlayPlayer(MediaPlayerEntity):
    ...
    async def async_update(self) -> None:
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                repeat_mode_str = data.get("repeat", "off").lower()
                if repeat_mode_str == "off":
                    self._attr_repeat = RepeatMode.OFF
                elif repeat_mode_str == "one":
                    self._attr_repeat = RepeatMode.ONE
                elif repeat_mode_str == "all":
                    self._attr_repeat = RepeatMode.ALL
                else:
                    self._attr_repeat = RepeatMode.OFF

                shuffle_val = data.get("shuffle", False)
                if isinstance(shuffle_val, bool):
                    self._attr_shuffle = shuffle_val
                else:
                    self._attr_shuffle = False
        except Exception as e:  # pragma: no cover
    ...
    async def async_repeat(self, repeat: RepeatMode) -> None:
        """Set repeat mode."""
        repeat_str = repeat.value
        success = await self._call_api("POST", f"/media_player/airplay_{self._device_name}/repeat", {"repeat": repeat_str})
        if success:
    ...
    async def async_shuffle(self, shuffle: bool) -> None:
        """Set shuffle mode."""
        success = await self._call_api("POST", f"/media_player/airplay_{self._device_name}/shuffle", {"shuffle": shuffle})
        if success:
