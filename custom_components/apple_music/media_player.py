        ...
        for i, delay in enumerate(delays):
            try:
                ...
                async with timeout(5):
                    url = f"{self._base_url}/artwork" + (f"?tok={quote(tok)}" if tok else "")
                    async with self._session.get(url) as resp:
                        if resp.status == 200:
                            ...
                            if data:
                                ...
                                # Treat SVG placeholder or ultra-small payloads as invalid; fall back to last-good
                                is_svg = (isinstance(ctype, str) and ctype.lower() == "image/svg+xml") or (data[:5] == b"<svg ")
                                too_small = len(data) < 128  # guard against tiny placeholders
                                if not is_svg and not too_small:
                                    # Cache last-good
