...
async def async_setup_entry(
    ...
    def _on_airplay_devices(device_data: list[dict]) -> None:
        ...
        for player_name, player in airplay_players.items():
            player_data = next((d for d in device_data if d.get('name') == player_name), None)
            if player_data:
                # Update availability and state
                player._available = player_data.get('active', False)  # type: ignore
...
class AppleMusicAirPlayPlayer(MediaPlayerEntity):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry, device_name: str) -> None:
        ...
        self._attr_should_poll = False
        # Dispatcher subscription for updates
        self._unsub = None
    ...
    @property
    def suggested_object_id(self) -> str:
        # Clean AirPlay device entity ID: simplified device_name + _music_controller
        device_slug = slugify(self._device_name)
        # Remove any "_airplay" suffixes to avoid duplication
        if device_slug.endswith('_airplay'):
            device_slug = device_slug[:-8]
        # Remove "_airplay" from anywhere in the name to clean it up
        device_slug = device_slug.replace('_airplay', '')
