...
class AppleMusicAirPlayPlayer(MediaPlayerEntity):
    ...
    @property
    def suggested_object_id(self) -> str:
        # Clean AirPlay device entity ID: device_name + _airplay_music_controller
        # Remove duplicate consecutive words in the device name and trailing "_airplay"
        ...
        # Remove trailing '_airplay' if present, then append '_airplay_music_controller'
        if clean_slug.endswith('_airplay'):
            clean_slug = clean_slug[:-8]
        return f"{clean_slug}_airplay_music_controller"
