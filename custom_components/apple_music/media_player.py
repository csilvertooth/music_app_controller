...
class AppleMusicPlayer(MediaPlayerEntity):
    ...
    def _search_results_to_browse(
        ...
        for key, mclass, prefix in key_map:
            ...
            for item in items:
                ...
                # Build a thumbnail URL directly from the controller (no HA proxy)
                thumb = None
    ...
    async def async_get_media_image(self) -> tuple[bytes | None, str | None]:
        ...
        # Prefer tokened URL when available for freshness
        tok = getattr(self, "_last_artwork_token", None)
        # Note: keep last-good cache across token changes to avoid transient blanks
        # Track token seen for debug only
        if tok != getattr(self, "_artwork_token_seen", None):
            self._artwork_last_fallback_used = False
            self._artwork_token_seen = tok
        # Some backends lag a moment after track-change; add short retries.
        delays = [0.0, 0.25, 0.5, 0.75, 1.0, 1.5]
        for i, delay in enumerate(delays):
            try:
                if delay:
                    await asyncio.sleep(delay)
                async with timeout(5):
                    base = f"{self._base_url}/artwork"
                    url = base + (f"?tok={quote(tok)}" if tok else "")
                    async with self._session.get(url) as resp:
                        if resp.status == 200:
                            ...
                            if data:
                                ...
                                # Treat SVG placeholder as invalid; accept tiny PNG/JPEG (proxy can handle it)
                                is_svg = (isinstance(ctype, str) and ctype.lower() == "image/svg+xml") or (data[:5] == b"<svg ")
                                if not is_svg:
                                    # Cache last-good
                                    self._last_art_bytes = data
                                    self._last_art_ctype = ctype
                                    return data, ctype
                                # Try a forced refresh once for small/SVG responses
                                try:
                                    rurl = url + ("&refresh=1" if "?" in url else "?refresh=1")
                                    async with self._session.get(rurl) as r2:
                                        if r2.status == 200:
                                            data2 = await r2.read()
                                            if data2 and len(data2) >= 128:
                                                c2 = r2.headers.get("Content-Type", "image/jpeg")
                                                if isinstance(c2, str) and ";" in c2:
                                                    c2 = c2.split(";", 1)[0].strip()
                                                self._last_art_bytes = data2
                                                self._last_art_ctype = c2
                                                return data2, c2
                                except Exception:
                                    pass
                                # else, continue retries
        ...
        # Resiliency: if we have a last-good image, return it instead of blanking
        if self._last_art_bytes and self._last_art_ctype:
            return self._last_art_bytes, self._last_art_ctype
        # Final fallback: tiny PNG to prevent HA media proxy from returning 500
        return _BLANK_PNG, "image/png"
